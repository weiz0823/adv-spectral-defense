...
def convert2cpu(gpu_matrix: Tensor):
    return torch.FloatTensor(gpu_matrix.size()).copy_(gpu_matrix)


def parse_cfg(cfgfile: str) -> list[dict[str, str]]:
    blocks = []
    fp = open(cfgfile, "r")
    block = None
    line = fp.readline()
...
def print_cfg(blocks: list[dict[str, str]]):
...
def load_conv(buf: ndarray, start: int, conv_model: _ConvNd):
...
def save_conv(fp, conv_model: _ConvNd):
...
def load_conv_bn(buf: ndarray, start: int, conv_model: _ConvNd, bn_model: _BatchNorm):
    num_w = conv_model.weight.numel()
...
def save_conv_bn(fp, conv_model: _ConvNd, bn_model: _BatchNorm):
...
def load_fc(buf: ndarray, start: int, fc_model: Linear):
...
def save_fc(fp, fc_model: Linear):
