...
class Darknet(nn.Module):
    ...
    def forward(self, x: Tensor, begin=0, end=None):
        ind = begin - 2
        self.loss = None
        outputs = dict()
        for block in self.blocks[begin:end]:
            ...
            if block["type"] == "net":
                ...
                or block["type"] == "maxpool"
                or block["type"] == "reorg"
                or block["type"] == "avgpool"
                or block["type"] == "softmax"
                or block["type"] == "connected"
            ):
                x = self.models[ind](x)
                outputs[ind] = x
    ...
    def create_network(self, blocks: list[dict[str, str]]):
    ...
    def load_weights(self, weightfile: str):
    ...
    def save_weights(self, outfile: str, cutoff=0):
        if cutoff <= 0:
