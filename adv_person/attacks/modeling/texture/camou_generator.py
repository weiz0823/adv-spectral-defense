...
def blend(img1, img2):
    ...
    img_blended_color = (
        img1[:, :3, :, :] * img1[:, 3, :, :](1 - img2[:, 3, :, :])
        + img2[:, :3, :, :] * img2[:, 3, :, :]
    )
    img_blended_alpha = (
        img1[:, 3, :, :] + img2[:, 3, :, :] - img1[:, 3, :, :] * img2[:, 3, :, :]
    )
...
def random_mask(figsize, num_geometry, prev_mask=None):
    ...
    for i in range(num_geometry):
        ...
        if x == 3:
            ...
            draw.polygon(
                [
                    center[0] + offsets[0, 0],
                    center[1] + offsets[0, 1],
                    center[0] + offsets[1, 0],
                    center[1] + offsets[1, 1],
                    center[0] + offsets[2, 0],
                    center[1] + offsets[2, 1],
                ],
                fill=(1, 1, 1),
            )
        img_np += np.array(img)
...
def drawtriangles(origin_triangles, coordinates, fig_size):
    coordinates = coordinates.expand(origin_triangles.shape[0], -1, -1, -1).permute(
        1, 2, 0, 3
    )
    ...
    s = (
        ...
        * (triangles[:, 1] - triangles[:, 0])
        / torch.norm(triangles[:, 1] - triangles[:, 0])
    )
    s = s.sum(dim=-1)
    l1 = s.le(0) * torch.norm(coordinates - triangles[:, 0], dim=-1)
    l1 += s.ge(torch.norm(triangles[:, 1] - triangles[:, 0])) * torch.norm(
        coordinates - triangles[:, 1], dim=-1
    )
    ...
    s = (
        ...
        * (triangles[:, 2] - triangles[:, 1])
        / torch.norm(triangles[:, 2] - triangles[:, 1])
    )
    s = s.sum(dim=-1)
    l2 = s.le(0) * torch.norm(coordinates - triangles[:, 1], dim=-1)
    l2 += s.ge(torch.norm(triangles[:, 2] - triangles[:, 1])) * torch.norm(
        coordinates - triangles[:, 2], dim=-1
    )
    ...
    s = (
        ...
        * (triangles[:, 0] - triangles[:, 2])
        / torch.norm(triangles[:, 0] - triangles[:, 2])
    )
    s = s.sum(dim=-1)
    l3 = s.le(0) * torch.norm(coordinates - triangles[:, 2], dim=-1)
    l3 += s.ge(torch.norm(triangles[:, 0] - triangles[:, 2])) * torch.norm(
        coordinates - triangles[:, 0], dim=-1
    )
...
def drawcircles(original_circles, coordinates, fig_size, alpha=None):
    coordinates = coordinates.expand(original_circles.shape[0], -1, -1, -1).permute(
        1, 2, 0, 3
    )
...
def drawcircles_fix_color(
    original_circles, coordinates, colors, fig_size_h, fig_size_w, blur=1
):
    assert original_circles.shape[0] == colors.shape[0]
    coordinates = coordinates.expand(original_circles.shape[1], -1, -1, -1).permute(
        1, 2, 0, 3
    )
    circle0 = original_circles[..., 0] * fig_size_h
    circle1 = original_circles[..., 1] * fig_size_w
    circles = torch.stack([circle0, circle1], dim=-1)
    dist_sum = torch.zeros([colors.shape[0], fig_size_h, fig_size_w]).to(
        coordinates.device
    )
...
def ctrl_loss(circles, fig_h, fig_w, sigma=40):
    ...
    diff_ell2 = (
        diff[..., 0] * diff[..., 0] * fig_h * fig_h
        + diff[..., 1] * diff[..., 1] * fig_w * fig_w
    )
