...
def get_points(infos, dt=0, dr=0, bs=1, wrap=True, random=True, pp_ratio=0.1):
    centers = (
        torch.tensor(infos["centers"]).view(-1, 1, 2).expand(-1, bs, -1)
    )  # [each size is (C, B, 2)]
    Rs = (
        torch.tensor(infos["Rs"]).view(-1, 1, 1).expand(-1, bs, -1)
    )  # [each size is (C, B, 1)]

    thetas_fixed = [
        torch.linspace(0, torch.pi * 2, ntf + 1)[:-1] for ntf in infos["ntfs"]
    ]
    radius_fixed = [torch.tensor(rf) for rf in infos["radius_fixed"]]

    thetas_wrap = [
        torch.linspace(0, torch.pi * 2, ntw + 1)[:-1] for ntw in infos["ntws"]
    ]
    radius_wrap = [torch.tensor(rw) for rw in infos["radius_wrap"]]

    param_fixed = [
        torch.cartesian_prod(tf, rf)[None, :].expand(bs, -1, -1)
        for tf, rf in zip(thetas_fixed, radius_fixed)
    ]
    param_wrap = [
        torch.cartesian_prod(tf, rf)[None, :].repeat(bs, 1, 1)
        for tf, rf in zip(thetas_wrap, radius_wrap)
    ]  # use repeat to avoid memory error

    nparts = infos["nparts"]
    if wrap:
        if random:
            dt = centers.new(size=[bs, nparts]).uniform_(-1, 1) * dt
            dr = centers.new(size=[bs, nparts]).uniform_(-1, 1) * dr
            for i in range(nparts):
                param_wrap[i][..., 0] += (
                    dt[:, i : i + 1]
                    + torch.zeros_like(param_wrap[i][..., 0]).uniform_(-1, 1)
                    * dt[:, i : i + 1]
                    * pp_ratio
                )
                param_wrap[i][..., 1] *= (
                    1
                    + dr[:, i : i + 1]
                    + torch.zeros_like(param_wrap[i][..., 1]).uniform_(-1, 1)
                    * dr[:, i : i + 1]
                    * pp_ratio
                )
        else:
            for i in range(nparts):
                param_wrap[i][..., 0] += (
                    torch.zeros_like(param_wrap[i][..., 0]).fill_(infos["signs"][i])
                    * dt
                )
                param_wrap[i][..., 1] *= (
                    1
                    + torch.zeros_like(param_wrap[i][..., 1]).fill_(infos["signs"][i])
                    * dr
                )
    ...
    thetas = [
        torch.stack([p[..., 0].cos(), p[..., 0].sin()], -1) for p in params
    ]  # [each size is (B, N, 2)]
    radius = [p[..., 1:2] for p in params]

    target_control_points = [centers.permute(1, 0, 2)] + [
        R * r * t + c
        for c, R, r, t in zip(centers[:, :, None], Rs[:, :, None], radius, thetas)
    ]
...
def fragments_reprojection(fragments, start, end, mesh, locations, infos):
    ...
    # compute coords bins
    indexes = (
        ((coords + infos["max_range"]) / infos["bin_size"])
        .round()
        .clamp(0, infos["bin_num"])
        .long()
    )
    indexes = indexes[:, 0] * (infos["bin_num"] + 1) + indexes[:, 1]
    bc_true = (
        F.pad(coords, [0, 1], value=1.0)
        .view(-1, 1, 1, 3)
        .matmul(grids_bc_kernels[indexes])
        .squeeze(-2)
    )

    indicator = (bc_true[..., 0] >= 0).logical_and(
        (bc_true[..., 1] >= 0).logical_and(bc_true[..., 2] >= 0)
    )
...
def my_hard_rgb_blend(colors: torch.Tensor, fragments, blend_params) -> torch.Tensor:
    """
...
def view_mesh_wrapped(
    ...
    **kwargs,
    ...
    if len(mesh_join) == 1 and len(cameras) > 1:
        ...
        print(
            "mesh num %d and camera %d num mis-match" % (len(mesh_join), len(cameras))
        )
        raise ValueError
    ...
    blend_params = (
        BlendParams(1e-4, 1e-4, background) if background is not None else None
    )
        ...
        for mesh, locations, infos in zip(mesh_list, locations_list, infos_list):
            ...
            if locations is not None:
                for i in range(len(mesh_join)):
                    fragments = fragments_reprojection(
                        fragments,
                        start + i * num_faces,
                        end + i * num_faces,
                        mesh,
