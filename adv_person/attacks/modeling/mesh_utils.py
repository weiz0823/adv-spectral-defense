...
def get_map_kernel(
    ...
    for fi, faces_uvs in enumerate(faces_uvs_list):
        ...
        area2 = (
            (
                (faces_locs[:, 0] - faces_locs[:, 1])
                * (faces_locs[:, 0] - faces_locs[:, 2]).flip(-1)
            )
            .sum(-1)
            .abs()
            .unsqueeze(0)
        )
        dis_to_tri = torch.where(
            area1 > area2, dis_to_tri, dis_to_tri.new([0.0])
        )  # Ng * B
    ...
    values = collect[:, 1]
    num_ids = F.pad(
        counts.unsqueeze(1).expand(-1, len(ids)).triu().sum(0)[:-1], [1, 0], value=0
    )
    ...
    kernel_fake = locations.new(
        [
            [max_range + 1, max_range + 1],
            [max_range + 2, max_range + 1],
    ...
    grids_bc_kernels_tt = torch.where(
        grids_index_tt[..., None, None].expand(-1, -1, 3, 3) >= 0,
        bc_kernel[grids_index_tt],
        kernel_fake[None, None, :],
    )
...
def update_mesh(mesh):
    ...
    if any(
        v is not None for v in [mesh._faces_areas_packed, mesh._faces_normals_packed]
    ):
...
def transforms(mesh, vector=None, angle=None, center=(0, 0, 0), scale=None):
    ...
    if angle is not None:
        ...
        b = angle.new(center)
        mesh._verts_packed = b + (mesh._verts_packed - b).matmul(
            mesh._verts_packed.new(
...
def join_meshes(meshes, join_maps=None):
    ...
    for i in range(len(meshes)):
        verts_uvs[i] = torch.stack(
            [
                (verts_uvs[i][:, 0] * maps[i].shape[1] + hi) / h,
                verts_uvs[i][:, 1] * maps[i].shape[0] / w,
            ],
            -1,
        )
