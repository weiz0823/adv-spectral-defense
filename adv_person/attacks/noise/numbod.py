...
class NumbOD(Adversary):
    ...
    def __init__(
        ...
        self.DWTl = DWT_2D_tiny(wavename=wave)
        self.IDWTl = IDWT_2D_tiny(wavename=wave)
        self.DWTh = DWT_2D(wavename=wave)
        self.IDWTh = IDWT_2D(wavename=wave)
    ...
    def _attack(
        ...
        inputs_ll = self.DWTl(X)
        inputs_ll = self.IDWTl(inputs_ll)
        inputs_ll0, inputs_lh, inputs_hl, inputs_hh1 = self.DWTh(X)
        inputs_hh = self.IDWTh(
            torch.zeros_like(inputs_ll0),
            torch.zeros_like(inputs_lh),
            torch.zeros_like(inputs_hl),
            inputs_hh1,
        )
        ...
        for _ in range(self.iterations):
            ...
            for (boxes, labels), targets in zip(boxes_with_labels, y):
                ...
                adv_ll = self.DWTl(X_adv)
                adv_ll = self.IDWTl(adv_ll)
                adv_ll0, adv_lh, adv_hl, adv_hh = self.DWTh(X_adv)
                adv_hh = self.IDWTh(
                    torch.zeros_like(adv_ll0),
                    torch.zeros_like(adv_lh),
                    torch.zeros_like(adv_hl),
                    adv_hh,
                )
                lowFre_cost = self.smooth_l1(adv_ll, inputs_ll)
                highFre_cost = -self.smooth_l1(adv_hh, inputs_hh)

                losses.append(
                    iou_loss + self.lambda_cls * cls_loss + lowFre_cost + highFre_cost
                )
